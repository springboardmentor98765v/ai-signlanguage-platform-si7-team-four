"""
Reconciled ORM models (aligns Backend with Database_Devops/app/db/models.py).

Source of truth is the Database_Devops schema (native PostgreSQL UUID keys),
with the Backend runtime requirements re-applied on top:
  - User.is_active        (admin_router status toggles)
  - User.instructor_id    (instructor_router assignment)
  - Notification.title    (notification_service / notification_router)
  - Module.description    + created_at (course.py seed + module ordering)
All primary/foreign keys use UUID(as_uuid=False) so the schema matches the
real PostgreSQL deployment; on SQLite the UUID type compiles to text.
"""
import uuid
...
class User(Base):
    ...
    # Backend admin/auth + instructor-assignment support.
...
class Module(Base):
    ...
    # Backend course.py seed + module ordering.
